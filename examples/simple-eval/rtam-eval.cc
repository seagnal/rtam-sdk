...
CT_RTAM_EVAL::CT_RTAM_EVAL(void) {
  ...
  _i_status_sample_error_cnt = 0;
	...
	/* Context */
	_pc_context_main = CT_GUARD<CT_HOST_CONTEXT>(new CT_HOST_CONTEXT);
...
int CT_RTAM_EVAL::f_send(CT_GUARD<CT_PORT_NODE> const & in_rpc_node) {
  bml::node_socket_writer<CT_GUARD> c_writer(_i_sock_fd);
  in_rpc_node->to_writer(c_writer);
  return EC_SUCCESS;
}

int CT_RTAM_EVAL::f_loop(void) {
    ...
    _i_sock_fd = socket(AF_UNIX, SOCK_STREAM, 0);
    if (_i_sock_fd < 0) {
        perror("socket");
        exit(EXIT_FAILURE);
    }
    ...
    if (connect(_i_sock_fd, (struct sockaddr*)&server_addr, sizeof(struct sockaddr_un)) < 0) {
        perror("connect");
        close(_i_sock_fd);
        exit(EXIT_FAILURE);
    }
    ...
    // Execute post init once connected
    f_post_init();

    bml::node_socket_parser<CT_GUARD> c_parser(_i_sock_fd);
    struct timeval timeout;
    
    // Polling loop using select
    while (1) {
        FD_ZERO(&read_fds);
        FD_SET(_i_sock_fd, &read_fds);
        ...
        int activity = select(_i_sock_fd + 1, &read_fds, NULL, NULL, &timeout);
        if (activity < 0 && errno != EINTR) {
            perror("select");
            break;
        } else if (activity == 0) {
            _DBG << "select timed out. No data received within the specified time";
            continue; // Continue the loop to try again
        }

        if (FD_ISSET(_i_sock_fd, &read_fds)) {

            CT_PORT_NODE_GUARD c_node;
            ec = c_node->from_parser(c_parser);
            if(ec == EC_BML_FAILURE) {
              perror("read");
              _FATAL << "Error during decoding";
            } else if (ec == EC_BML_NODATA) {
              _CRIT<< "End of Data";
            } 

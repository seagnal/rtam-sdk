 ...
 ***********************************************************************/
 ...
 typedef bml::node<uint32_t, CT_GUARD> CT_PORT_NODE;

 class CT_HOST_CONTEXT {
public:
    int f_start(std::function<int(CT_HOST_CONTEXT&)> in_f_run) {
        // Simulate starting a context
        std::thread thread([in_f_run, this]() {
            in_f_run(*this);
        });
...
// Define the macro M_CONTEXT_BIND
#define M_CONTEXT_BIND(Class, Method, Instance) std::bind( &Class::Method, Instance, std::placeholders::_1)
...
public:
	CT_PORT_NODE_GUARD() :
			CT_GUARD<CT_PORT_NODE>(new CT_PORT_NODE()) {
	}
...
public:
	...
	int f_loop(void);
	int f_send(CT_GUARD<CT_PORT_NODE> const & in_rpc_node);
